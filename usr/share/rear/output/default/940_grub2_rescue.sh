...
if is_true $USING_UEFI_BOOTLOADER ; then
    ...
    local grub_module=""
    local grub_modules=""
    for grub_module in part_gpt part_msdos fat ext2 normal chain boot configfile linux linuxefi multiboot jfs iso9660 usb usbms usb_keyboard video udf ntfs all_video gzio efi_gop reboot search test echo btrfs lvm ; do
    ...
    if ! grub${grub_num}-mkimage $v -o $boot_dir/efi/EFI/BOOT/rear.efi -O x86_64-efi -c $grub_config_dir/rear_embed.cfg -p "/EFI/BOOT" $grub_modules ; then
        Error "Failed to make bootable EFI image for GRUB_RESCUE=y"
    fi
